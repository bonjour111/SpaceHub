...
class LaunchMapperImpl(
    ...
    private val launchStatusMapper: LaunchStatusMapper,
    ...
    override fun mapToDomain(launchLocal: LaunchLocal) = Launch(
        ...
        netDisplay = launchLocal.net?.let { dateParser.parseFullDate(it) },
        windowStartDisplay = launchLocal.windowStart?.let { dateParser.parseFullDate(it) },
        windowEndDisplay = launchLocal.windowEnd?.let { dateParser.parseFullDate(it) },
        windowEnd = launchLocal.windowEnd,
        netMillis = launchLocal.net?.let { dateParser.parseDateInMillis(it) },
        status = launchStatusMapper.mapToDomain(launchLocal.status),
    ...
    override fun mapToLocal(launchRemote: LaunchRemote, launchType: LaunchType?) = LaunchLocal(
        ...
        status = launchStatusMapper.mapToLocal(launchRemote.status),
        type = launchType,
